...
//! This is a modified copy of https://github.com/snail-with-tea/egui_virtual_keyboard
//!
//! -> Workaround for virtual keyboard on android - TODO: remove as soon winit etc. supports android keyboard.

use std::{
    error::Error, fmt::Display, num::ParseFloatError, ops::RangeInclusive, rc::Rc, str::FromStr,
};

use xframe::egui::{
    self, Rect, Response, RichText, Sense, StrokeKind, TextStyle, Ui, Vec2, WidgetText, vec2,
};
...
impl VirtualKeyboard {
    ...
    pub fn is_active(&self, ctx: &egui::Context) -> bool {
        ...
        !state.deactivate && (state.focus.is_some() || state.contains_pointer)
    }

    /// Surrender keyboard focus & remove keyboard from ui
    ...
    ///
    /// => When used with 'eframe' place this on the 'raw_input_hook'.
    pub fn bump_events(&mut self, raw_input: &mut egui::RawInput) {
        raw_input.events.append(&mut self.events);
    }

    /// Adds the Keyboard to ui if there is a text field focused.
    ///
    /// **Note:** The keyboard is represented in a `egui::Layout::bottom_up`, so be aware that
    /// a currently active `egui::Layout::bottom_up` area will be covered by the keyboard.
    pub fn show_if_active(&mut self, ui: &mut Ui) -> Response {
        let (cur_focused, cur_state) = ui.memory_mut(|m| {
            (
                m.focused(),
                m.data.remove_temp::<State>(self.id).unwrap_or_default(),
            )
        });

        if cur_state.deactivate {
            if let Some(id) = cur_state.focus {
        ...
        if let Some(id) = cur_focused {
            let resp = self.keyboard(ui);
            ui.memory_mut(|m| {
                m.data.insert_temp(
                    self.id,
                    State {
    ...
    fn keyboard(&mut self, ui: &mut Ui) -> Response {
        let resp = ui.with_layout(egui::Layout::bottom_up(egui::Align::LEFT), |wl_ui| {
            self.buttons(wl_ui)
        });
    ...
    fn draw_one(
        ...
        let text = WidgetText::RichText(Rc::new(RichText::new(&button.text).monospace()))
